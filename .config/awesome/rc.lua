...
local function check_updates()
  ...
  awful.spawn.easy_async_with_shell(command, function(output)
    local new_amount = string.gsub(output, '\n', '')
    amount = new_amount
    if new_amount ~= '0' then
      update_widget.text = ' ' .. amount
    end
    if amount == '0' then
      update_widget.text = ' '
    else
      local new_line = amount .. ' new updates available'
      awful.spawn('notify-send "Updates available" "' .. new_line .. '"')
      update_widget.markup = markup(beautiful.fg_minimize, ' ' .. amount)
    end
  end)
end
...
-- Update with mouse left, spawn terminal with right
...
-- My variables
internalGapTable = {4, 10, 30, 0}
internalGapIndex = 1
externalGapTable = {5, 36, 60, 90, 0}
externalGapIndex = 1
mySelectedTags = {}
tagsHidden = false
...
{description = "open flameshot gui", group = "launcher"}),

awful.key({modkey}, "b", function() awful.spawn("firefox") end,
{description = "open Firefox", group = "launcher"}),

awful.key({modkey, "Shift"}, "b", function() awful.spawn("firefox --private-window") end,
{description = "open Firefox in private", group = "launcher"}),
...
function() awful.client.incwfact(0.1) end,
...
awful.key({modkey}, "0",
function() awful.screen.focused().selected_tag.gap = internalGapTable[internalGapIndex]
  if internalGapIndex < 4 then
    internalGapIndex = internalGapIndex + 1
  else
    internalGapIndex = 1
  end
...
{description = "Cycle through internal gap sizes", group = "layout"}),

awful.key({modkey, "Shift"}, "0",
function() awful.screen.focused().padding = externalGapTable[externalGapIndex]
  if externalGapIndex < 5 then
...
awful.key({modkey, "Shift", "Control"}, "0",
function() 
  awful.screen.focused().padding = 0
  awful.screen.focused().selected_tag.gap = internalGapTable[1]
  -- Update the layout for all monitors
  awful.screen.connect_for_each_screen(function(s)
    awful.layout.arrange(s)
  end)
end,
{description = "Reset external and internal gaps to default", group = "layout"}),

awful.key({modkey}, "d",
function() 
  if tagsHidden == false then
    awful.screen.connect_for_each_screen(function(s)
      for index, tag in ipairs(s.selected_tags) do
        table.insert(mySelectedTags, tag)
        tag.selected = false
      end
    end)
    tagsHidden = true
    ...
    awful.screen.connect_for_each_screen(function(s)
      for index, tag in ipairs(mySelectedTags) do
        tag.selected = true
      end
    end)
    mySelectedTags = {}
...
{description = "Hide/show all selected tags", group = "layout"}),
...
awful.key({modkey}, "i", function()
  ...
  client.focus:raise()
end, {description = "focus master client", group = "client"}),
...
awful.key({modkey, "Shift"}, "w", function(c) if c.pid then awful.spawn("kill -9 " .. c.pid) end end,
{description = "Kill program",	group = "client"}),
